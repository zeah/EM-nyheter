...
final class EmArtPager {
	...
	public function add_meta_struc_callback() {
		...
		if (json_decode($this->getmeta('emartstruc')))
			echo '<textarea style="width: 100%; height: 20em;" name="emartstruc">'.json_encode(json_decode($this->getmeta('emartstruc')), JSON_PRETTY_PRINT).'</textarea>';
		else
			echo '<textarea style="width: 100%; height: 20em;" name="emartstruc">'.$this->getmeta('emartstruc').'</textarea>';
	}
	...
	public function savemeta($postid) {
		...
		foreach ($meta as $value)
			if (isset($_POST[$value]))
				update_post_meta($postid, $value, sanitize_text_field($_POST[$value]));
	}
	...
	public function add_footer() {
		global $post;
		if ($post && $post->post_type == 'article') 
			echo json_decode($this->getmeta('emartstruc')) ? '<script type="application/ld+json">'.json_encode(json_decode($this->getmeta('emartstruc'))).'</script>' : '';
	}
}
