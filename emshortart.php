...
final class ShortEmArt {
	...
	*/
	public function shortcode_callback($atts, $content = null) {
		...
		if (isset($atts['nr']) && is_numeric($atts['nr']))
				$args['posts_per_page'] = intval($atts['nr']);
		...
		if (sizeof($p) > 0) {
			...
			$html .= '</a></li>';
			if (is_active_sidebar('emarticle-widget'))
				$html .= '<li class="em-articles-listitem em-articles-widget"><ul class="em-articles-widget-list">'.$sidebar.'</ul></li>';
		}
	...
	public function add_to_footer($name) {
		...
		// echo '<script defer>(function () { var o = document.createElement("link");o.setAttribute("rel", "stylesheet");o.setAttribute("href", "'.ARTICLE_PLUGIN_URL.'assets/emart-style2.css");o.setAttribute("media", "(min-width:1000px)");document.head.appendChild(o); })();</script>';
